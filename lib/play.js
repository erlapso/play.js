  ...
  */
  ...
  Play.prototype.sound = function sound (file, callback) {
    ...
    if (!this.playerName && this.checked === this.playerList.length) {
      ...
      console.log('If you know other cmd line music player than these:', this.playerList);
      console.log('You can tell us, and will add them (or you can add them yourself)');
    ...
    child.on('exit', function (code, signal) {
      if(code == null || signal != null || code === 1) {
        console.log('couldnt play, had an error ' + '[code: '+ code + '] ' + '[signal: ' + signal + '] :' + this.playerName);
//        this.emit('error', code, signal);
        callback(new Error(code));
      }
      else if ( code == 127 ) {
        // a could not find command error
        console.log( self.playerName + ' doesn\'t exist!' );
        this.emit('error', code, signal);
        callback(new Error(code));
      }
      else if (code == 2) {
        // a file IO error
        console.log(file + '=>' + 'could not be read by your player:' + self.playerName)
        this.emit('error', code, signal);
        callback(new Error(code));
      }
      else if (code == 0) {
        ...
        console.log( self.playerName + ' has an odd error with ' + file);
